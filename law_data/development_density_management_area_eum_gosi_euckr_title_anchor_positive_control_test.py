...
OUT = BASE / "law_data" / "output" / "development_density_management_area_eum_gosi_euckr_title_anchor_positive_control.json"
...
# Both controls are title-field anchors for zonenm, not detail-body terms.
CONTROLS = [
    {
        "query": "분당지구단위계획",
        "expected_seq": "638968",
        "expected_notice": "2026-121",
        "expected_title_fragment": "분당지구단위계획",
    },
    {
        "query": "주차장, 공원, 유수지, 지구단위계획",
        "expected_seq": "632588",
        "expected_notice": "2026-87",
        "expected_title_fragment": "주차장, 공원, 유수지, 지구단위계획",
    },
]
...
def main() -> None:
    print("=" * 60)
    print("EUM GOSI EUC-KR TITLE-ANCHOR POSITIVE CONTROL - S172")
    print("=" * 60)
    print("zonenm semantics: TITLE FIELD ONLY")
    ...
    for c in CONTROLS:
        ...
        title_anchor_ok = c["expected_title_fragment"] in text
        qualified = (
        ...
        row = {
            ...
            "title_anchor_ok": title_anchor_ok,
            "encoded_query_fragment": enc_component(c["query"]),
    ...
    qualified_count = sum(1 for r in results if r["state"] == "EUM_GOSI_EUCKR_TITLE_ANCHOR_QUALIFIED")
    technical = sum(1 for r in results if r["state"] == "TECHNICAL_REQUEST_UNKNOWN")
    unresolved = sum(1 for r in results if r["state"] == "EUM_GOSI_EUCKR_TITLE_ANCHOR_NOT_RESOLVED")

    out = {
        "step": "STEP 17-21-C-16-8-T-68-S172",
        "target_name": "개발밀도관리구역",
        "standard_code": "UQQ700",
        "source_family": "NATIONAL_LAND_USE_PORTAL",
        "search_endpoint": URL,
        "search_transport": "WARM_SESSION_EUC_KR_FORM_URLENCODED",
        "search_field_semantics": "zonenm=TITLE",
        "results": results,
        "summary": {
            "positive_control_count": len(results),
            "title_anchor_qualified_count": qualified_count,
    ...
    checks = {
        ...
        "all title anchors qualified": qualified_count == len(CONTROLS),
    ...
    if not all(checks.values()):
        raise AssertionError("S172 EUM EUC-KR title-anchor positive control failed")
