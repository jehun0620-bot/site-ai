...
def get_text(s,seq):
    ...
    for enc in ('euc-kr','cp949','utf-8'):
        try:return raw.decode(enc),str(r.url)
        except UnicodeDecodeError:pass
    return raw.decode('euc-kr',errors='ignore'),str(r.url)
...
def post_file(s,url,data,referer):
    try:
        ...
        return {'state':'HTTP_RESPONSE_CAPTURED' if not ov else 'TECHNICAL_REQUEST_UNKNOWN','http':r.status_code,'final_url':str(r.url),'content_type':r.headers.get('Content-Type'),'content_disposition':r.headers.get('Content-Disposition'),'content_length':len(b),'signature_hex':bytes(b[:16]).hex(),'error':'RESPONSE_SIZE_LIMIT_EXCEEDED' if ov else None}
    except requests.RequestException as e:return {'state':'TECHNICAL_REQUEST_UNKNOWN','http':None,'final_url':url,'content_type':None,'content_disposition':None,'content_length':0,'signature_hex':'','error':f'{type(e).__name__}: {e}'}

def main():
    print('='*60);print('EUM GOSI ATTACHMENT POST CONTRACT QUALIFICATION - S181');print('='*60)
    print('Text extraction: DISABLED');print('OCR: DISABLED');print('Negative evidence: DISABLED');print('UQQ700 resolution: UNKNOWN')
    results=[]
    for seq in SAMPLES:
        s=requests.Session();s.headers.update({'User-Agent':UA,'Accept-Language':'ko-KR,ko;q=0.9'})
        text,detail_url=get_text(s,seq)
        dls=DL_RE.findall(text)
        ...
        if not dls:
            row={'seq':seq,'state':'ATTACHMENT_DOWNLOAD_CALL_NOT_FOUND','download_call_count':0,'hidden_fields':hidden}; results.append(row); print('SEQ:',seq,'| STATE:',row['state']); continue
        ...
        r=post_file(s,target,payload,detail_url)
        official=(urlparse(r['final_url']).hostname or '').endswith('eum.go.kr')
        delivered=r['state']=='HTTP_RESPONSE_CAPTURED' and r['http']==200 and r['content_length']>0 and official
        state='ATTACHMENT_POST_DELIVERY_QUALIFIED' if delivered else ('TECHNICAL_REQUEST_UNKNOWN' if r['state']=='TECHNICAL_REQUEST_UNKNOWN' else 'ATTACHMENT_POST_DELIVERY_NOT_RESOLVED')
        row={'seq':seq,'state':state,'download_call_count':len(dls),'download_url':target,'file_id':file_id,'hidden_fields':hidden,'official_host':official,**r}; results.append(row)
        print('SEQ:',seq,'| STATE:',state,'| HTTP:',r['http'],'| TYPE:',r['content_type'],'| BYTES:',r['content_length'],'| SIG:',r['signature_hex'])
    q=sum(x['state']=='ATTACHMENT_POST_DELIVERY_QUALIFIED' for x in results); tech=sum(x['state']=='TECHNICAL_REQUEST_UNKNOWN' for x in results); unresolved=len(results)-q-tech
    out={'step':'STEP 17-21-C-16-8-T-77-S181','target_name':'개발밀도관리구역','standard_code':'UQQ700','results':results,'summary':{'sample_count':len(results),'qualified_count':q,'technical_unknown_count':tech,'unresolved_count':unresolved,'semantic_state':'EUM_ATTACHMENT_POST_CONTRACT_QUALIFIED' if q==len(results) else 'EUM_ATTACHMENT_POST_CONTRACT_NOT_YET_QUALIFIED','negative_evidence_allowed':False,'legal_absence_inference_allowed':False,'uqq700_final_resolution':'UNKNOWN'},'attachment_text_extraction_executed':False,'ocr_allowed':False,'site_positive_allowed':False,'site_negative_allowed':False,'runtime_registration_allowed':False}
    OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
    print('\nSUMMARY');[print(f'{k}: {v}') for k,v in out['summary'].items()];print('Output:',OUT)
    checks={'sample exact':len(results)==len(SAMPLES),'technical unknown zero':tech==0,'all qualified':q==len(results),'text extraction disabled':not out['attachment_text_extraction_executed'],'OCR disabled':not out['ocr_allowed'],'negative evidence disabled':not out['summary']['negative_evidence_allowed'],'legal absence inference disabled':not out['summary']['legal_absence_inference_allowed'],'unsafe promotion leakage zero':not any(out[k] for k in ['site_positive_allowed','site_negative_allowed','runtime_registration_allowed']),'final resolution unknown':out['summary']['uqq700_final_resolution']=='UNKNOWN','output written':OUT.exists() and OUT.stat().st_size>0}
    print('\nVALIDATION');[print(f'{k}: {v}') for k,v in checks.items()];print('all_pass:',all(checks.values()))
    if not all(checks.values()): raise AssertionError('S181 EUM attachment POST contract qualification failed')
