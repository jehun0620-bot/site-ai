# -*- coding: utf-8 -*-
"""S71: quarantine Gazette 1868/1870 as cluster-consistent technical attachment unresolved."""
...
OUT=OUTDIR/'development_density_management_area_municipal_gazette_1868_1870_cluster_quarantine_reconciliation.json'
TARGETS=['343615','343834']
...
def main():
    print('='*60); print('GAZETTE 1868/1870 CLUSTER QUARANTINE RECONCILIATION - S71'); print('='*60)
    ...
    for pst in TARGETS:
        if pst not in by: raise AssertionError(f'missing target {pst}')
        row=by[pst]
        if row.get('status')!='EXTRACTION_OR_REQUEST_UNKNOWN':
            raise AssertionError(f'unexpected prior status {pst}: {row.get("status")}')
    ...
    if not all(vals.values()): raise AssertionError('S71 reconciliation failed')
