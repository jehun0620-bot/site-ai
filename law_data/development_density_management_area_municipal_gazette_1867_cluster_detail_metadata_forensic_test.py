# -*- coding: utf-8 -*-
"""S68: detail/metadata forensic for the 2023-06~07 attachment-empty cluster around Gazette 1867."""
...
DETAIL_URL='https://www.seongnam.go.kr/city/1000052/30005/bbsView.do?idx={pst}'


def norm(v): return str(v or '').strip()

def main():
    print('='*60); print('GAZETTE 1867 CLUSTER DETAIL/METADATA FORENSIC - S68'); print('='*60)
    ...
    for pst in TARGETS:
        ...
        file_nos=sorted(set(re.findall(r'fileNo\s*[=:]\s*["\']?([0-9]+)',text,re.I)))
        concrete_names=sorted(set(re.findall(r'[^<>"\']+\.(?:hwp|hwpx|pdf|xls|xlsx)',text,re.I)))
        plumbing={k:(k in text) for k in ['/bbs010308/atchFileDetail','/getFile','/filePreview']}
        rec={'gazette_number':row.get('gazette_number'),'date':norm(row.get('date')),'pstSn':pst,'metadata_http':hs,'metadata_url':mu,'metadata_attachment_count':len(attachments),'metadata_attachments':attachments,'detail_http':resp.status_code,'detail_url':url,'detail_title':title,'detail_html_bytes':len(resp.content or b''),'dynamic_attachment_plumbing':plumbing,'concrete_file_nos':file_nos,'concrete_file_names':concrete_names}
    ...
    summary={'target_count':len(records),'all_metadata_empty':all_metadata_empty,'all_detail_http_200':all_detail_ok,'same_detail_attachment_plumbing_pattern':same_detail_pattern,'all_without_concrete_attachment_refs':no_concrete_refs,'cluster_consistent_technical_pattern':all_metadata_empty and all_detail_ok and same_detail_pattern and no_concrete_refs}
    payload={'step':'STEP 17-21-C-16-8-T-34-S68','records':records,'summary':summary,'attachment_body_download_executed':False,'state_mutation_executed':False,'negative_evidence_allowed':False,'site_positive_allowed':False,'site_negative_allowed':False,'runtime_registration_allowed':False}
    ...
    if not all(vals.values()): raise AssertionError('S68 forensic validation failed')
