...
OUT = BASE / "law_data" / "output" / "development_density_management_area_eum_gosi_search_wire_replay_positive_control.json"
...
def main() -> None:
    print("=" * 60)
    print("EUM GOSI SEARCH WIRE REPLAY POSITIVE CONTROL - S165")
    print("=" * 60)
    print("Browser form semantics: geul/gihyung/silsi=Y, pageNo='' ")
    ...
    for c in CONTROLS:
        ...
        state = "EUM_GOSI_SEARCH_WIRE_REPLAY_QUALIFIED" if qualified else ("TECHNICAL_REQUEST_UNKNOWN" if res["state"] == "TECHNICAL_REQUEST_UNKNOWN" else "EUM_GOSI_SEARCH_WIRE_REPLAY_NOT_RESOLVED")
        row = {
            "query": c["query"], "expected_seq": c["expected_seq"], "expected_notice": c["expected_notice"],
            "state": state, "http": res["http"], "official_host": official, "encoding": encoding,
    ...
    qualified_count = sum(1 for r in results if r["state"] == "EUM_GOSI_SEARCH_WIRE_REPLAY_QUALIFIED")
    technical = sum(1 for r in results if r["state"] == "TECHNICAL_REQUEST_UNKNOWN")
    unresolved = sum(1 for r in results if r["state"] == "EUM_GOSI_SEARCH_WIRE_REPLAY_NOT_RESOLVED")

    out = {
        "step": "STEP 17-21-C-16-8-T-61-S165",
        "target_name": "개발밀도관리구역",
        "standard_code": "UQQ700",
        "source_family": "NATIONAL_LAND_USE_PORTAL",
        "search_endpoint": URL,
        "search_transport": "POST_FORM_BROWSER_SEMANTIC_REPLAY",
        "wire_contract": {
            "pageNo": "",
            "geul_yn": "Y",
            "gihyung_yn": "Y",
        ...
        "summary": {
            ...
            "semantic_state": "EUM_GOSI_SEARCH_WIRE_CONTRACT_QUALIFIED" if qualified_count == len(CONTROLS) else "EUM_GOSI_SEARCH_WIRE_CONTRACT_NOT_YET_QUALIFIED",
    ...
    if not all(checks.values()):
        raise AssertionError("S165 EUM gosi search wire replay positive control failed")
