...
OUT = BASE / "law_data" / "output" / "development_density_management_area_eum_gosi_canonical_wire_positive_control.json"
...
def bounded_post(session: requests.Session, data: dict[str, str]) -> dict:
    try:
        r = session.post(
            URL,
            data=data,
            headers={"Referer": URL, "Origin": "https://www.eum.go.kr"},
            timeout=25,
            stream=True,
...
def main() -> None:
    print("=" * 60)
    print("EUM GOSI CANONICAL WIRE POSITIVE CONTROL - S167")
    print("=" * 60)
    print("Canonical list-state keys: S166-derived")
    ...
    for c in CONTROLS:
        ...
        state = "EUM_GOSI_CANONICAL_WIRE_QUALIFIED" if qualified else ("TECHNICAL_REQUEST_UNKNOWN" if res["state"] == "TECHNICAL_REQUEST_UNKNOWN" else "EUM_GOSI_CANONICAL_WIRE_NOT_RESOLVED")
        row = {
            "query": c["query"],
    ...
    qualified_count = sum(1 for r in results if r["state"] == "EUM_GOSI_CANONICAL_WIRE_QUALIFIED")
    technical = sum(1 for r in results if r["state"] == "TECHNICAL_REQUEST_UNKNOWN")
    unresolved = sum(1 for r in results if r["state"] == "EUM_GOSI_CANONICAL_WIRE_NOT_RESOLVED")

    out = {
        "step": "STEP 17-21-C-16-8-T-63-S167",
        "target_name": "개발밀도관리구역",
        "standard_code": "UQQ700",
        "source_family": "NATIONAL_LAND_USE_PORTAL",
        "search_endpoint": URL,
        "search_transport": "POST_FORM_CANONICAL_LIST_STATE_REPLAY",
        "canonical_keys": list(CANONICAL_FORM.keys()),
        "results": results,
        "summary": {
            "positive_control_count": len(results),
            "canonical_wire_qualified_count": qualified_count,
    ...
    checks = {
        ...
        "all canonical wire controls qualified": qualified_count == len(CONTROLS),
    ...
    if not all(checks.values()):
        raise AssertionError("S167 EUM canonical wire positive control failed")
