# -*- coding: utf-8 -*-
"""
STEP 17-21-C-16-8-T-16-S8-S2
Development Density Management Area
Cumulative Historical Row Coverage Expansion - Batch 2

S8 + S8-S1에서 이미 조회한 page를 누적 제외하고 다음 미확인 page를 제한 순회한다.
Detail request는 실행하지 않으며 row-local direct 개발밀도관리구역 identity만 candidate로 인정한다.
"""
...
S8S1_PATH = BASE_DIR / "law_data" / "output" / "development_density_management_area_competent_authority_historical_row_coverage_expansion.json"
...
OUT_PATH = OUT_DIR / "development_density_management_area_competent_authority_historical_row_coverage_expansion_2.json"
...
def parse_rows(raw_html: str, family: str, page_url: str, page_number: int) -> List[Dict[str, Any]]:
    ...
    for tr_index, match in enumerate(TR_PATTERN.finditer(raw_html or ""), start=1):
        ...
        args = unique_strings(m.group(1) for m in call_pattern.finditer(row_html))
        if not args:
            continue
        ...
        dates = [f"{int(y):04d}-{int(m):02d}-{int(d):02d}" for y, m, d in DATE_PATTERN.findall(row_text)]
...
def load_prior_pages(*datasets: Dict[str, Any]) -> Set[tuple[str, int]]:
...
def main() -> None:
    ...
    print("CUMULATIVE HISTORICAL ROW COVERAGE EXPANSION - BATCH 2")
    print("=" * 60)

    for path in (S8_PATH, S8S1_PATH, T15S2_PATH):
        if not path.exists():
            raise FileNotFoundError(f"input missing: {path}")

    s8 = json.loads(S8_PATH.read_text(encoding="utf-8"))
    s8s1 = json.loads(S8S1_PATH.read_text(encoding="utf-8"))
    t15 = json.loads(T15S2_PATH.read_text(encoding="utf-8"))

    prior_pages = load_prior_pages(s8, s8s1)

    contracts: List[Dict[str, Any]] = []
    seen_contracts: Set[tuple[str, str, str]] = set()
    for item in t15.get("next_stage_boundary_pool") or []:
        if not isinstance(item, dict):
            continue
        family = normalize_space(item.get("source_family"))
    ...
    for index, contract in enumerate(contracts, start=1):
        missing = [
            page for page in range(contract["lower"], contract["upper"] + 1)
        ...
        print("Missing page count:", len(missing))
        print("Selected pages:", selected)

        for page in selected:
            if request_count >= MAX_TOTAL_REQUESTS:
                break
            url = build_page_url(contract["base_url"], contract["pagination_key"], page)
            ...
            rows = []
            if isinstance(status, int) and 200 <= status < 300:
                rows = parse_rows(str(response.get("text") or ""), contract["source_family"], response.get("final_url") or url, page)
    ...
    duplicate_removed = 0
    for row in all_rows:
        key = (row["source_family"], row["argument"])
    ...
    candidates = [x for x in canonical_rows if x.get("target_identity_direct") is True]

    cumulative_pages = set(prior_pages)
    cumulative_pages.update((x["source_family"], int(x["page_number"])) for x in page_results)
    remaining_coverage = []
    ...
    output_data = {
        "step": "STEP 17-21-C-16-8-T-16-S8-S2 Cumulative Historical Row Coverage Expansion Batch 2",
        ...
        "summary": {
            ...
            "uqq700_candidate_count": len(candidates),
            "remaining_coverage": remaining_coverage,
        },
        "page_results": page_results,
        "canonical_rows": canonical_rows,
        "uqq700_candidates": candidates,
        "next_stage_document_candidate_pool": candidates,
        "resolution": (
            "COMPETENT_AUTHORITY_CUMULATIVE_COVERAGE_UQQ700_CANDIDATE_RECOVERED"
            if candidates
            else "COMPETENT_AUTHORITY_CUMULATIVE_COVERAGE_NO_UQQ700_ROW"
        ),
        "verified_positive": False,
        "runtime_registration_allowed": False,
        "site_positive_allowed": False,
    ...
    unsafe_leakage = sum(
        1 for x in canonical_rows
        if x.get("verified_positive") is True
        ...
        or x.get("detail_request_executed") is True
    )
    invalid_host_leakage = sum(1 for x in page_results if not is_government_host(hostname(x.get("page_url") or "")))

    validations = {
        "inputs exist": S8_PATH.exists() and S8S1_PATH.exists() and T15S2_PATH.exists(),
        "prior coverage loaded": len(prior_pages) > 0,
        "request budget respected": request_count <= MAX_TOTAL_REQUESTS,
        "already-covered page requery leakage zero": all((x["source_family"], int(x["page_number"])) not in prior_pages for x in page_results),
    ...
    print("UQQ700 candidates:", len(candidates))
    ...
    if not all(validations.values()):
        raise AssertionError("cumulative coverage expansion batch 2 regression failed")
