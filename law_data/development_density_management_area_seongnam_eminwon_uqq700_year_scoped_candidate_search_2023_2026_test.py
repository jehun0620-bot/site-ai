...
OUT = BASE / "law_data" / "output" / "development_density_management_area_seongnam_eminwon_uqq700_year_scoped_candidate_search_2023_2026.json"
...
def make_form(year: str, spec: dict, page: int) -> dict[str, str]:
    data = dict(BASE_FORM)
    data["yyyy"] = year
    data["Key"] = spec["key"]
...
def extract_candidates(text: str, year: str, spec: dict, page: int) -> list[dict]:
    chunks = ROWISH_RE.findall(text) or [text]
    out = []
    seen = set()
    for chunk in chunks:
        ...
        for mgt_no in sorted(ids):
            key = (mgt_no, year, page, spec["field"], spec["query"])
            if key in seen:
                continue
            seen.add(key)
...
def main() -> None:
    print("=" * 60)
    print("SEONGNAM EMINWON UQQ700 YEAR-SCOPED CANDIDATE SEARCH 2023-2026 - S156")
    print("=" * 60)
    print("Year filter contract: QUALIFIED BY S153")
    print("Prior 2003-2012 tranche: CLOSED BY S154")
    print("Prior 2013-2022 tranche: CLOSED BY S155")
    print("Years:", YEARS)
    print("Max pages/year/query:", MAX_PAGES_PER_YEAR_QUERY)
    ...
    for year in YEARS:
        print("\nYEAR:", year)
        for spec in QUERIES:
            print(" QUERY:", spec["field"], "|", spec["query"])
            for page in range(1, MAX_PAGES_PER_YEAR_QUERY + 1):
                data = make_form(year, spec, page)
                ...
                year_echo = year in text
                candidates = extract_candidates(text, year, spec, page) if res["state"] == "HTTP_RESPONSE_CAPTURED" else []
                rec = {
                    "year": year,
                    "field": spec["field"],
                    "query": spec["query"],
                ...
                print("  PAGE:", page, "| STATE:", rec["state"], "| HTTP:", rec["http"], "| Q_ECHO:", query_echo, "| Y_ECHO:", year_echo, "| CANDIDATES:", len(candidates))

                if res["state"] == "TECHNICAL_REQUEST_UNKNOWN":
                    stop_reason = "TECHNICAL_REQUEST_UNKNOWN"
                    break

                high = [c for c in candidates if c["status"] in ("DIRECT_CANDIDATE", "RELATED_CANDIDATE")]
                if high:
                    stop_reason = "HIGH_SIGNAL_CANDIDATE_FOUND"
                    break

                if page > 1 and len(candidates) == 0:
                    break
    ...
    for c in all_candidates:
        ...
        if k not in dedup or (dedup[k]["status"] == "SEARCH_RESULT_CANDIDATE" and c["status"] != "SEARCH_RESULT_CANDIDATE"):
            dedup[k] = c
    ...
    technical = sum(1 for r in request_log if r["state"] == "TECHNICAL_REQUEST_UNKNOWN")

    if technical:
        semantic = "SEONGNAM_EMINWON_2023_2026_UQQ700_SEARCH_TECHNICAL_UNKNOWN"
    elif direct or related:
        semantic = "SEONGNAM_EMINWON_2023_2026_UQQ700_HIGH_SIGNAL_CANDIDATE_FOUND"
    elif candidates:
        semantic = "SEONGNAM_EMINWON_2023_2026_UQQ700_SEARCH_RESULT_CANDIDATES_FOUND"
    else:
        semantic = "SEONGNAM_EMINWON_2023_2026_UQQ700_NO_CANDIDATE_IN_BOUNDED_SEARCH_SURFACE"

    out = {
        "step": "STEP 17-21-C-16-8-T-52-S156",
        ...
        "search_contract": {
            ...
            "year_field": "yyyy",
            "page_size": PAGE_SIZE,
            "max_pages_per_year_query": MAX_PAGES_PER_YEAR_QUERY,
        },
    ...
    ceiling = len(YEARS) * len(QUERIES) * MAX_PAGES_PER_YEAR_QUERY
    checks = {
        "bounded request ceiling respected": len(request_log) <= ceiling,
    ...
    if not all(checks.values()):
        raise AssertionError("S156 year-scoped UQQ700 candidate search 2023-2026 failed")
