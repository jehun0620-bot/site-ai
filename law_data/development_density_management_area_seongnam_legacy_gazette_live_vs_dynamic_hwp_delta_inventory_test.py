...
represented in the existing dynamic-HWP state. It reads list identity/year only and local
state JSON only. It does not request details/attachments or search UQQ700 terms.
"""
...
def main():
 ...
 if state_path is None: raise FileNotFoundError('dynamic-HWP state JSON not found')
 state=json.loads(state_path.read_text(encoding='utf-8')); prior_ids=extract_pstsn(state)
 print('STATE:',state_path); print('PRIOR PSTSN COUNT:',len(prior_ids))
