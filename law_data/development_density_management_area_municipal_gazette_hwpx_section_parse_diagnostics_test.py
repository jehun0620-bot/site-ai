...
Download exactly one validated representative HWPX, persist that bounded sample for
subsequent offline forensics, and diagnose why section*.xml fails XML parsing.
...
def main():
 ...
 if raw:
  try:
   ...
   zip_ok=False;err=(err+"; " if err else "")+f"ZIP_ERROR {e!r}"
 if zip_ok and raw:
  SAMPLE.write_bytes(raw)
 out={"step":"STEP 17-21-C-16-8-T-28-S1-1 Municipal Gazette HWPX Section Parse Diagnostics","target":{"name":TARGET_NAME,"standard_code":STANDARD_CODE},"request":{"count":1,"params":params},"response":{"http_status":status,"final_url":final_url,"response_bytes":len(raw),"error":err},"archive":{"zip_valid":zip_ok,"section_count":len(diagnostics),"persisted_sample":str(SAMPLE),"persisted_sample_bytes":SAMPLE.stat().st_size if SAMPLE.exists() else 0},"section_diagnostics":diagnostics,"bulk_traversal_executed":False,"verified_positive":False,"site_positive_allowed":False,"site_negative_allowed":False,"resolution":"MUNICIPAL_GAZETTE_HWPX_SECTION_PARSE_DIAGNOSTICS_COMPLETED"}
 OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding="utf-8")
 vals={"T-26 input exists":T26.exists(),"T-27 input exists":T27.exists(),"single bounded download":out["request"]["count"]==1,"HTTP 200":status==200,"official host":gov(host(final_url)) and host(final_url)==host(c["action"]),"HWPX ZIP valid":zip_ok,"section diagnostics recovered":len(diagnostics)>0,"representative sample persisted":SAMPLE.exists() and SAMPLE.stat().st_size==len(raw)>0,"bulk traversal disabled":not out["bulk_traversal_executed"],"unsafe promotion disabled":not any([out["verified_positive"],out["site_positive_allowed"],out["site_negative_allowed"]]),"output written":OUT.exists() and OUT.stat().st_size>0}
 print("HTTP:",status);print("Final URL:",final_url);print("Response bytes:",len(raw));print("ZIP valid:",zip_ok);print("Section count:",len(diagnostics));print("Persisted sample:",SAMPLE)
