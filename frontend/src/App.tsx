  ...
  return (
    <main className={`app-shell${resultReady ? ' app-shell-result-ready' : ''}`}>
      <section className="search-panel" aria-labelledby="page-title">
        ...
        {analysisState !== 'IDLE' && <section className={`analysis-panel analysis-${analysisState.toLowerCase()}`} aria-live="polite"><div className="analysis-title-row"><strong>{analysisState === 'ANALYSIS_READY' ? 'SITE 분석 결과' : analysisState === 'ANALYSIS_FAILED' ? 'SITE 분석 실패' : 'SITE 분석 중'}</strong>{analysisState === 'ANALYSIS_READY' && <span className="analysis-ready-badge">분석 완료</span>}</div><p>{analysisMessage}</p>{analysis && <div className="analysis-details">
          <section className="analysis-detail-section"><h2>필지 기본정보</h2><dl className="analysis-summary"><div><dt>지번주소</dt><dd>{displayValue(analysis.site.address)}</dd></div><div><dt>도로명주소</dt><dd>{displayValue(analysis.site.road_address)}</dd></div><div><dt>PNU</dt><dd>{displayValue(analysis.site.pnu)}</dd></div><div><dt>용도지역</dt><dd>{displayValue(analysis.site.zone)}</dd></div><div><dt>분석 상태</dt><dd>{displayAnalysisStatus(analysis.status)}</dd></div><div><dt>추가 입력</dt><dd>{analysis.requirements.requires_additional_input ? '필요' : '현재 응답 기준 없음'}</dd></div></dl></section>
