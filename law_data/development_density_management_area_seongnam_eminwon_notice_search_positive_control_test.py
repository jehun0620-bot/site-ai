...
OUT = BASE / "law_data" / "output" / "development_density_management_area_seongnam_eminwon_notice_search_positive_control.json"
...
def main():
    print("=" * 60)
    print("SEONGNAM EMINWON NOTICE SEARCH POSITIVE CONTROL - S149")
    print("=" * 60)
    ...
    for control in CONTROLS:
        ...
        row = {
            ...
            "state": "SEARCH_CONTRACT_QUALIFIED" if qualified else ("TECHNICAL_REQUEST_UNKNOWN" if res["state"] == "TECHNICAL_REQUEST_UNKNOWN" else "SEARCH_POSITIVE_CONTROL_NOT_RESOLVED"),
            "http": res["http"],
    ...
    qualified_count = sum(1 for r in results if r["state"] == "SEARCH_CONTRACT_QUALIFIED")
    technical_count = sum(1 for r in results if r["state"] == "TECHNICAL_REQUEST_UNKNOWN")
    unresolved_count = sum(1 for r in results if r["state"] == "SEARCH_POSITIVE_CONTROL_NOT_RESOLVED")

    out = {
        "step": "STEP 17-21-C-16-8-T-45-S149",
        "target_name": "개발밀도관리구역",
        "standard_code": "UQQ700",
        "source_family": "SEONGNAM_EMINWON_HISTORICAL_NOTICE",
        "search_endpoint": URL,
        "search_transport": "POST_FORM",
        "title_field": "not_ancmt_sj",
        "content_field": "not_ancmt_cn",
        ...
        "summary": {
            ...
            "semantic_state": "SEONGNAM_EMINWON_NOTICE_SEARCH_CONTRACT_QUALIFIED" if qualified_count == len(CONTROLS) else "SEONGNAM_EMINWON_NOTICE_SEARCH_CONTRACT_NOT_YET_QUALIFIED",
    ...
    if not all(checks.values()):
        raise AssertionError("S149 eminwon search positive control failed")
