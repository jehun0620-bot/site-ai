...
OUT = BASE / "law_data" / "output" / "development_density_management_area_e_gazette_target_query_replay.json"
...
TARGET = "개발밀도관리구역"
TARGET_QUERY = "(unstored_field_subject:(개발밀도관리구역)) AND keyword_category_order:(@@ORDER_NUM)"
...
def main():
    print("=" * 78)
    print("E-GAZETTE UQQ700 TARGET QUERY REPLAY - S221H")
    print("=" * 78)
    print("Target query:", TARGET)
    print("Scope: page 1 bounded discovery only")
    ...
    summary_g = s221g.get("summary") or {}
    gate_pc = summary_g.get("positive_control_replay_qualified") is True
    gate_semantic = summary_g.get("semantic_state") == "E_GAZETTE_SEARCH_REST_API_POSITIVE_CONTROL_QUALIFIED"
    gate_safe = summary_g.get("uqq700_query_replayed") is False and summary_g.get("uqq700_final_resolution") == "UNKNOWN"
    if not (gate_pc and gate_semantic and gate_safe):
        raise AssertionError("S221H prerequisite gate not satisfied")
    ...
    for category_index, category in enumerate(categories):
        ...
        for item_index, item in enumerate(items):
            flat = flatten_strings(item)
            matches = [(p, v) for p, v in flat if TARGET in v]
            if matches:
                cat_matches += 1
        ...
        category_summaries.append({
            ...
            "matched_item_count": cat_matches,
            "needs_pagination_if_targeted": count > len(items) and count > 0,
        })

    technical_unknown = not response_contract_signal
    ...
    semantic = {
        "TARGET_HIT": "E_GAZETTE_UQQ700_TARGET_HIT_DISCOVERY_ONLY",
        "TARGET_NO_HIT": "E_GAZETTE_UQQ700_TARGET_NO_HIT_WITHOUT_LEGAL_ABSENCE_INFERENCE",
        "TECHNICAL_UNKNOWN": "E_GAZETTE_UQQ700_TARGET_QUERY_TECHNICAL_UNKNOWN",
    }[classification]

    if classification == "TARGET_HIT":
        next_action = "QUALIFY_E_GAZETTE_TARGET_CANDIDATE_DOCUMENT_IDENTITIES_BEFORE_ANY_LEGAL_PROMOTION"
    elif classification == "TARGET_NO_HIT":
        next_action = "EXPAND_E_GAZETTE_QUERY_SCOPE_OR_SUBJECTDESC_WITHOUT_LEGAL_ABSENCE_INFERENCE"
    else:
        next_action = "REPAIR_OR_RECHECK_E_GAZETTE_TARGET_QUERY_TRANSPORT_BEFORE_ANY_INTERPRETATION"

    out = {
        "step": "STEP 17-21-C-16-8-T-124-S221H",
        "target_name": TARGET,
        "standard_code": "UQQ700",
        "resolution_type": "HYBRID_SPATIAL_NOTICE",
        "source_family": "E_GAZETTE",
        "input_s221g": str(S221G),
    ...
    checks = {
        "S221G positive-control gate": gate_pc and gate_semantic,
        "entry GET 200": entry.status_code == 200,
        "search page GET 200": search_page.status_code == 200,
        "target query performed": out["summary"]["query_replay_performed"] is True,
    ...
    if not all(checks.values()):
        raise AssertionError("S221H e-gazette UQQ700 target query replay failed")
