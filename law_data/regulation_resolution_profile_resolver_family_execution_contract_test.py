...
from .regulation_resolution_profile_resolver_family_execution import EXECUTED, REJECTED, execute_admitted_resolver_family
...
def _execute(chain, resolver_input):
    ...
    return execute_admitted_resolver_family(plan, resolver_input, admitted_profile=profile, seed=seed, evidence=evidence, verification=verification)


def main() -> None:
    historical_chain = _execution_chain(name="도시지역편입해제구역", condition_type="SITE_HISTORY", resolution_type="HISTORICAL_SITE_EVENT")
    hybrid_chain = _execution_chain(name="개발밀도관리구역", condition_type="SITE", resolution_type="HYBRID_SPATIAL_NOTICE")
    district_unit_chain = _execution_chain(name="지구단위계획", condition_type="SITE", resolution_type="HYBRID_SPATIAL_NOTICE")

    historical = _execute(historical_chain, HistoricalSiteEventEvidenceState(verified_qualifying_event_present=True))
    assert historical.status == EXECUTED and historical.executed is True
    assert historical.resolver_family == "HISTORICAL_SITE_EVENT"
    assert historical.resolver_output is not None and historical.resolver_output["resolution_type"] == "HISTORICAL_SITE_EVENT" and historical.resolver_output["resolution"] == "TRUE_CANDIDATE"
    ...
    district_unit = _execute(district_unit_chain, hybrid_input)
    for result in (hybrid, district_unit):
        assert result.status == EXECUTED and result.executed is True
        assert result.resolver_family == "HYBRID_SPATIAL_NOTICE"
        assert result.resolver_output is not None and result.resolver_output["resolution_type"] == "HYBRID_SPATIAL_NOTICE"
    assert district_unit_chain[3].standard_code is None
    assert district_unit_chain[3].standard_code_verified is False
    ...
    assert plan_only.status == REJECTED and plan_only.executed is False
    cross_input = _execute(hybrid_chain, HistoricalSiteEventEvidenceState())
    assert cross_input.status == REJECTED and cross_input.executed is False
    cross_provenance = execute_admitted_resolver_family(hybrid_chain[4], hybrid_input, admitted_profile=hybrid_chain[3], seed=historical_chain[0], evidence=historical_chain[1], verification=historical_chain[2])
    assert cross_provenance.status == REJECTED and cross_provenance.executed is False
    forged_plan = RegulationResolutionProfileResolverFamilyDispatchPlan(status=PLANNED, resolver_family="HYBRID_SPATIAL_NOTICE", classification_compatible=True)
    forged = execute_admitted_resolver_family(forged_plan, hybrid_input)
    assert forged.status == REJECTED and forged.executed is False
    escalated_plan = replace(hybrid_chain[4], resolver_execution_allowed=True)
    escalated = execute_admitted_resolver_family(escalated_plan, hybrid_input, admitted_profile=hybrid_chain[3], seed=hybrid_chain[0], evidence=hybrid_chain[1], verification=hybrid_chain[2])
    assert escalated.status == REJECTED and escalated.executed is False
    arbitrary = _execute(hybrid_chain, {"resolver": "HYBRID_SPATIAL_NOTICE"})
    assert arbitrary.status == REJECTED and arbitrary.executed is False
    invalid = execute_admitted_resolver_family(None, hybrid_input)  # type: ignore[arg-type]
    assert invalid.status == REJECTED and invalid.executed is False
