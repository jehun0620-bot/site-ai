...
from .regulation_resolution_profile_site_decision_eligibility import ELIGIBLE, INELIGIBLE, REJECTED, evaluate_site_decision_eligibility
...
def _eligibility(chain):
    ...
    return evaluate_site_decision_eligibility(execution, plan, resolver_input, admitted_profile=profile, seed=seed, evidence=evidence, verification=verification)
...
def main() -> None:
    ...
    historical_true_candidate = _chain(name="도시지역편입해제구역", condition_type="SITE_HISTORY", resolution_type="HISTORICAL_SITE_EVENT", resolver_input=HistoricalSiteEventEvidenceState(verified_qualifying_event_present=True))
    historical_unknown = _chain(name="도시지역편입해제구역", condition_type="SITE_HISTORY", resolution_type="HISTORICAL_SITE_EVENT", resolver_input=HistoricalSiteEventEvidenceState())
    ...
    assert eligible_false.status == ELIGIBLE and eligible_false.eligible is True and eligible_false.resolver_result_verified is True
    assert eligible_false.resolution == "FALSE" and eligible_false.candidate_site_decision is False and eligible_false.conclusive_for_site_decision is True
    true_candidate = _eligibility(historical_true_candidate)
    assert true_candidate.status == INELIGIBLE and true_candidate.eligible is False and true_candidate.resolver_result_verified is True
    assert true_candidate.resolution == "TRUE_CANDIDATE" and true_candidate.candidate_site_decision is None
    unknown = _eligibility(historical_unknown)
    assert unknown.status == INELIGIBLE and unknown.eligible is False and unknown.resolution == "UNKNOWN"
    hybrid_result = _eligibility(hybrid)
    assert hybrid_result.status == INELIGIBLE and hybrid_result.eligible is False and hybrid_result.resolution == "UNKNOWN"
    district_unit_result = _eligibility(district_unit)
    assert district_unit_result.status == INELIGIBLE and district_unit_result.eligible is False
    assert district_unit_result.resolution == "UNKNOWN" and district_unit_result.candidate_site_decision is None
    ...
    for result in (eligible_false, true_candidate, unknown, hybrid_result, district_unit_result):
    ...
    no_provenance = evaluate_site_decision_eligibility(historical_false[6], historical_false[4], historical_false[5])
    assert no_provenance.status == REJECTED and no_provenance.eligible is False
    cross_provenance = evaluate_site_decision_eligibility(historical_false[6], historical_false[4], historical_false[5], admitted_profile=historical_false[3], seed=hybrid[0], evidence=hybrid[1], verification=hybrid[2])
    assert cross_provenance.status == REJECTED and cross_provenance.eligible is False
    forged_execution = replace(historical_true_candidate[6], resolver_output={**(historical_true_candidate[6].resolver_output or {}), "resolution": "FALSE", "resolution_basis": "VERIFIED_EXHAUSTIVE_DISPROOF"})
    forged = evaluate_site_decision_eligibility(forged_execution, historical_true_candidate[4], historical_true_candidate[5], admitted_profile=historical_true_candidate[3], seed=historical_true_candidate[0], evidence=historical_true_candidate[1], verification=historical_true_candidate[2])
    assert forged.status == REJECTED and forged.eligible is False
    escalated_execution = replace(historical_false[6], site_promotion_allowed=True)
    escalated = evaluate_site_decision_eligibility(escalated_execution, historical_false[4], historical_false[5], admitted_profile=historical_false[3], seed=historical_false[0], evidence=historical_false[1], verification=historical_false[2])
    assert escalated.status == REJECTED and escalated.eligible is False
    invalid = evaluate_site_decision_eligibility(None, historical_false[4], historical_false[5], admitted_profile=historical_false[3], seed=historical_false[0], evidence=historical_false[1], verification=historical_false[2])  # type: ignore[arg-type]
    assert invalid.status == REJECTED and invalid.eligible is False
